...
`tools/cuda/run_acceptance_matrix.py` automates the required AI/RA/LD x 8/10 CPU/CUDA matrix using external immutable inputs and sequence configs. AI defaults to one frame; RA and LD default to, and require, at least four. The versioned report hashes the runner, executables, profile configs, sequence configs, and inputs; records separate stdout/stderr logs and commands for one warm-up plus five measured runs; rejects nondeterministic repeats; compares encoder bitstreams/reconstructions and decoder YUV byte-for-byte; and records timings and medians. CUDA acceptance additionally requires QPA dispatch in every profile, SAD dispatch in inter profiles, loop-filter-chain dispatch in every decode, and zero reported failures, fallbacks, poisoning, or not-eligible pictures. Missing telemetry is treated as a possible CPU fallback and fails with the saved log paths. Example:

```text
python tools/cuda/run_acceptance_matrix.py --encoder <EncoderApp> --decoder <DecoderApp> \
  --cfg-dir cfg --input-8 <8bit.yuv> --input-10 <10bit.yuv> \
  --sequence-config-8 <sequence-8.cfg> --sequence-config-10 <sequence-10.cfg> \
  --output-dir <external-output-directory>
```

The full external AI/RA/LD x 8/10 acceptance matrix has **not** been run as part of this change and is not claimed here. The runner's `--dry-run` mode was exercised on the checked-in 1920x1080 process fixture, and its telemetry parser was exercised both with focused positive/negative tests and with an actual CUDA fixture decode reporting one dispatch, zero failures, and zero not-eligible pictures.
